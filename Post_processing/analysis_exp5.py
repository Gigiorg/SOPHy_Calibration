...
PATH_5 = r"C:\Users\GIBS\Documents\Documents\SOPHy_Calibration\Processing\EXP5"


df = pd.ExcelFile(PATH_5+'\\'+'Table_exp5.xlsx').parse('hits')
...
if (f > 0):
    ...
    for i in sorted(pd.unique(df.Azimuth)):
        ...
        ax.set_title(f"Experiment 5, Azimuth: {i}°")
...
#Filtering to only the Max Power Samples
...
if (f > 0):
    ...
    for i in sorted(pd.unique(df.Azimuth)):
        ...
        c_initial = perf2['C_initial [dB]']
        c_after = perf2["C_after Wr [dB]"]
        c_after_wb = perf2["C_after Wb [dB]"]
        
        c_after_wb_lin = perf2["C_after Wb"]
        ...
        date_time = pd.to_datetime(perf2["Datetime"]) 
        
        theo_constant = perf2['Exp Constant [dB]']
        theo_constant_lin = perf2["Exp Constant"]
        ...
        ax.set_title(f"Experiment 2, Azimuth: {i}° - Only Max Power Echoes")
        ax.legend([ "Theoretical Constant",f"Fitted Experimental Constant, Offset: {round(exp_tick-10*np.log10(avg_c_after_wb),2)} dB" ,
                   f"Experimental Constant with Wr and Wb, Std: {std_after_wb}"])
...
#Filtering to only R Power values above certain threshold
...
if (f > 0):
    ...
    for i in sorted(pd.unique(df.Azimuth)):
        ...
        ax.set_title(f"Experiment 2, Azimuth: {i}° - Only Power Echoes over -15.0 dB")
        ax.legend([ "Theoretical Constant",f"Fitted Experimental Constant, Offset: {round(exp_tick-10*np.log10(avg_c_after_wb),2)} dB" ,
                   f"Experimental Constant with Wr and Wb, Std: {std_after_wb}"])
...
if (f > 0):
    ...
    for i in sorted(pd.unique(df.Azimuth)):
        ...
        plt.title(f"Experiment 5 - Scatter plot WrWb vs Pr*r^4 for Azimuth: {i}°")
        plt.xlabel("WrWb")
